 ...
 */
#define MAX_FD 98 // My implementation can only handle 98 files open at the same time
 ...
 * Changes on line 119: Commented that line out because it was a typo pointed out by the TA
 * Changes on lines 187-188: This change was done because file descriptors were hard coded to be 0 and 1
 * instead of using fds[0] and fds[1]. My implementation reserves file descriptor 0 for the root directory
 * Changes on line 197: This change was done because I was running into weird memory problems that I could not figure
...
//	sfs_remove(names[0]); commented this out since it's a typo by the TA
	for (i = 0; i < 2; i++) {
		for (j = i + 1; j < 2; j++) {
			if (fds[i] == fds[j]) {
				fprintf(stderr,
	...
	for (i = 0; i < 2; i++) {
		for (j = 0; j < filesize[i]; j += chunksize) {
			...
			if ((buffer = malloc(chunksize*(MAX_BYTES/2))) == NULL) { // changed here
				fprintf(stderr, "ABORT: Out of memory!\n");
				exit(-1);
			}
			...
			for (k = 0; k < readsize; k++) {
				if (buffer[k] != (char) (j + k)) {
					fprintf(stderr,
							"ERROR: data error at offset %d in file %s (%d,%d)\n",
	...
	if (fds[0] >= 0) {
		for (i = 0; i < 100000; i++) {
			...
			if (x != sizeof(fixedbuf)) {
				...
				printf(
						"If the emulated disk contains just over %d bytes, this is OK\n",
